...
CREATE INDEX IF NOT EXISTS idx_ai_generations_transformation_type 
...
CREATE INDEX IF NOT EXISTS idx_public_gallery_user_created 
ON public_gallery(user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_public_gallery_created_at 
...
CREATE INDEX IF NOT EXISTS idx_api_cost_records_created_at 
...
CREATE INDEX IF NOT EXISTS idx_user_cost_stats_last_updated 
...
CREATE INDEX IF NOT EXISTS idx_cost_alerts_alert_type 
...
CREATE INDEX IF NOT EXISTS idx_data_access_audit_user_created 
ON data_access_audit(user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_data_access_audit_action 
...
CREATE INDEX IF NOT EXISTS idx_usage_stats_user_date_count 
...
-- 19. 创建用于分页的索引
CREATE INDEX IF NOT EXISTS idx_ai_generations_pagination 
ON ai_generations(created_at DESC, id) 
WHERE status = 'completed';

-- 20. 为实时查询优化
CREATE INDEX IF NOT EXISTS idx_ai_generations_realtime 
...
-- 23. 为用户行为分析优化
CREATE INDEX IF NOT EXISTS idx_ai_generations_user_behavior 
ON ai_generations(user_id, transformation_type, created_at DESC);

-- 24. 为性能监控添加索引
CREATE INDEX IF NOT EXISTS idx_ai_generations_performance 
ON ai_generations(processing_time_ms, created_at DESC) 
WHERE processing_time_ms IS NOT NULL;

-- 25. 创建用于数据导出的索引
CREATE INDEX IF NOT EXISTS idx_ai_generations_export 
