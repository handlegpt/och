...
export const UserSettings: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    const avatarUrlValidation = settings.avatar_url ? validateUrl(settings.avatar_url) : { isValid: true };
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <h2 className="text-2xl font-bold text-[var(--accent-primary)] mb-6">{t('app.userSettings.title')}</h2>
      ...
      <div className="space-y-6">
        ...
        {activeTab === 'profile' && (
          <>
            ...
            <div className="bg-[var(--bg-card-alpha)] backdrop-blur-lg rounded-xl border border-[var(--border-primary)] p-6">
              <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-4">{t('app.userSettings.basicInfo')}</h3>
        ...
        {/* 偏好设置 */}
        <div className="bg-[var(--bg-card-alpha)] backdrop-blur-lg rounded-xl border border-[var(--border-primary)] p-6">
          <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-4">{t('app.userSettings.preferences')}</h3>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-end">
              ...
              >
                {loading ? t('app.userSettings.saving') : saved ? t('app.userSettings.saved') : t('app.userSettings.save')}
              </button>
            </div>
          </>
        )}
        ...
        {activeTab === 'account' && (
          <div className="space-y-6">
            {/* 账户信息 */}
            <div className="bg-[var(--bg-card-alpha)] backdrop-blur-lg rounded-xl border border-[var(--border-primary)] p-6">
              <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-4">{t('app.userSettings.account.title')}</h3>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-[var(--text-secondary)]">{t('app.userSettings.account.email')}</span>
                  <span className="text-[var(--text-primary)] font-medium">{user?.email}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[var(--text-secondary)]">{t('app.userSettings.account.registrationDate')}</span>
                  <span className="text-[var(--text-primary)] font-medium">
                    {user?.created_at ? new Date(user.created_at).toLocaleDateString('zh-CN') : '未知'}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[var(--text-secondary)]">{t('app.userSettings.account.subscriptionStatus')}</span>
            ...
            {/* 安全设置 */}
            <div className="bg-[var(--bg-card-alpha)] backdrop-blur-lg rounded-xl border border-[var(--border-primary)] p-6">
              <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-4">{t('app.userSettings.security.title')}</h3>
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <div>
                    <div className="text-[var(--text-primary)] font-medium">{t('app.userSettings.security.loginMethod')}</div>
                    <div className="text-sm text-[var(--text-secondary)]">Google OAuth / Magic Link</div>
                  </div>
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <div className="text-[var(--text-primary)] font-medium">{t('app.userSettings.security.twoFactorAuth')}</div>
            ...
            <div className="bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-800 p-6">
              <h3 className="text-lg font-semibold text-red-600 dark:text-red-400 mb-4">{t('app.userSettings.security.dangerZone')}</h3>
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <div>
                    <div className="text-red-600 dark:text-red-400 font-medium">{t('app.userSettings.security.deleteAccount')}</div>
                    <div className="text-sm text-red-500 dark:text-red-300">{t('app.userSettings.security.deleteAccountDesc')}</div>
                  </div>
        ...
        {activeTab === 'export' && (
          <div className="space-y-6">
            {/* 导出选项 */}
            <div className="bg-[var(--bg-card-alpha)] backdrop-blur-lg rounded-xl border border-[var(--border-primary)] p-6">
              <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-4">{t('app.userSettings.export.title')}</h3>
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  <div>
                    <div className="text-[var(--text-primary)] font-medium">{t('app.userSettings.export.generationHistory')}</div>
                    <div className="text-sm text-[var(--text-secondary)]">导出所有生成记录为 JSON 格式</div>
                  </div>
                  <button 
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <div className="text-[var(--text-primary)] font-medium">{t('app.userSettings.export.userSettings')}</div>
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <div className="text-[var(--text-primary)] font-medium">{t('app.userSettings.export.completeData')}</div>
            ...
            {/* 导入选项 */}
            <div className="bg-[var(--bg-card-alpha)] backdrop-blur-lg rounded-xl border border-[var(--border-primary)] p-6">
              <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-4">{t('app.userSettings.export.import.title')}</h3>
              <div className="space-y-4">
                <div>
