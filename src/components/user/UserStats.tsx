...
export const UserStats: React.FC = () => {
  ...
  const fetchUserStats = async () => {
    ...
    try {
      ...
      const transformationCounts = transformationData && transformationData.length > 0 
      ...
      const favoriteTransformation = transformationCounts && Object.keys(transformationCounts).length > 0
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-[var(--bg-secondary)] rounded-lg p-3">
          ...
          <div className="text-lg font-bold text-[var(--text-primary)]">{stats.totalGenerations}</div>
        </div>
        
        <div className="bg-[var(--bg-secondary)] rounded-lg p-3">
          <div className="text-xs text-[var(--text-secondary)]">本月生成</div>
          <div className="text-lg font-bold text-[var(--text-primary)]">{stats.thisMonthGenerations}</div>
        </div>
        
        <div className="bg-[var(--bg-secondary)] rounded-lg p-3">
          <div className="text-xs text-[var(--text-secondary)]">本周生成</div>
          <div className="text-lg font-bold text-[var(--text-primary)]">{stats.thisWeekGenerations}</div>
        </div>
        
        <div className="bg-[var(--bg-secondary)] rounded-lg p-3">
          <div className="text-xs text-[var(--text-secondary)]">今日生成</div>
          <div className="text-lg font-bold text-[var(--text-primary)]">{stats.todayGenerations}</div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {stats.favoriteTransformation && (
          <div className="flex justify-between items-center text-sm">
            ...
            <span className="text-[var(--text-primary)] font-medium">{stats.favoriteTransformation}</span>
          </div>
        )}
        ...
        {stats.lastGeneration && (
          <div className="flex justify-between items-center text-sm">
            ...
            <span className="text-[var(--text-primary)] font-medium">{formatDate(stats.lastGeneration)}</span>
          </div>
        )}
      </div>
    </div>
