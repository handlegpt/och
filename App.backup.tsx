...
import { DataPersistenceService } from './src/services/dataPersistence';
import { LoginPromptModal } from './src/components/LoginPromptModal';
import { MagicLinkModal } from './src/components/MagicLinkModal';
import { ErrorBoundary } from './src/components/ErrorBoundary';
import { validatePrompt, generationRateLimiter } from './src/utils/security';
...
const App: React.FC = () => {
  ...
  const handleGenerateImage = useCallback(async () => {
    ...
    try {
        ...
        if (selectedTransformation.isTwoStep) {
            ...
            const stepOneResult = await editImage(primaryBase64, primaryMimeType, sanitizedPrompt, null, null, user?.id);
            ...
            const stepTwoResult = await editImage(stepOneImageBase64, stepOneImageMimeType, sanitizedPrompt, null, secondaryImagePayload, user?.id);
            ...
            const result = await editImage(primaryBase64, primaryMimeType, sanitizedPrompt, maskBase64, secondaryImagePayload, user?.id);
  ...
  }, [selectedTransformation, handleGenerateVideo, handleGenerateImage, user]);
